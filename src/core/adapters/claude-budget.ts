 ...
 *   Tier 3 — Unrecognized / unspecified: Sonnet-class default
 *     → maxPromptChars: 128_000, maxSystemPromptChars: 800_000
 */
...
export function getClaudePromptBudget(model?: string): {
  ...
  // Haiku workers always specify model: "haiku" explicitly — keep their conservative limits.
  if (model != null && /haiku/i.test(model)) {
    return { maxPromptChars: 32_768, maxSystemPromptChars: 180_000 };
  }

  // Undefined, empty, or unrecognized model — default to Sonnet-class budget.
  // Haiku workers always set model explicitly; unspecified workers run on Sonnet-class hardware.
  return { maxPromptChars: 128_000, maxSystemPromptChars: 800_000 };
}
