...
\${c}\`:c,s=s?\`\${s}
\${u}\`:u;let d=this.lexer.state.top;if(this.lexer.state.top=!0,this.lexer.blockTokens(u,a,!0),this.lexer.state.top=d,n.length===0)break;let p=a.at(-1);if(p?.type==="code")break;if(p?.type==="blockquote"){let m=p,h=m.raw+\`
\`+n.join(\`
\`),y=this.blockquote(h);a[a.length-1]=y,i=i.substring(0,i.length-m.raw.length)+y.raw,s=s.substring(0,s.length-m.text.length)+y.text;break}else if(p?.type==="list"){let m=p,h=m.raw+\`
\`+n.join(\`
\`),y=this.list(h);a[a.length-1]=y,i=i.substring(0,i.length-p.raw.length)+y.raw,s=s.substring(0,s.length-m.raw.length)+y.raw,n=h.substring(a.at(-1).raw.length).split(\`
