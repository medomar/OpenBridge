...
> **Pending:** 42 | **In Progress:** 0 | **Done:** 0
...
| 8   | OB-1007 | OB-F33  | Verify `npm run typecheck` passes — all 20 TS errors in `master-manager.ts` should auto-resolve from OB-1000/OB-1001                                                                                                                  | ◻ Pending |
...
| #   | Task ID | Finding | Description                                                                                                                                                                                            | Status    |
| --- | ------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------- |
| 22  | OB-1030 | OB-F35  | Add `listSessions(limit, offset)` to `conversation-store.ts` — returns `{ session_id, title, first_message_at, last_message_at, message_count, channel, user_id }[]` ordered by `last_message_at DESC` | ◻ Pending |
| 23  | OB-1031 | OB-F35  | Add `title` column to `conversations` table via migration — nullable `TEXT`, set to first user message (truncated 50 chars) on session creation                                                        | ◻ Pending |
| 24  | OB-1032 | OB-F35  | Add `searchSessions(query, limit)` to `conversation-store.ts` — FTS5 search that returns session-level results (grouped by `session_id`, ranked by relevance)                                          | ◻ Pending |
...
| #   | Task ID | Finding | Description                                                                                                                                                       | Status    |
| --- | ------- | ------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------- |
| 31  | OB-1040 | OB-F28  | Create `schema_versions` table (`version INTEGER PRIMARY KEY, applied_at TEXT, description TEXT`) in `database.ts`                                                | ◻ Pending |
| 32  | OB-1041 | OB-F28  | Number all existing migrations in `migration.ts` — assign version 1 through N to current `ALTER TABLE` sequences                                                  | ◻ Pending |
...
| #   | Task ID | Finding | Description                                                                                                                              | Status    |
| --- | ------- | ------- | ---------------------------------------------------------------------------------------------------------------------------------------- | --------- |
| 35  | OB-1050 | OB-F30  | Stream stdout chunks from active workers via `execOnceStreaming()` — parse turn indicators from CLI output                               | ◻ Pending |
| 36  | OB-1051 | OB-F30  | Broadcast `worker-progress` events to all connectors with `{ workerId, turnsUsed, turnsMax, lastAction }`                                | ◻ Pending |
| 37  | OB-1052 | OB-F30  | Add worker progress tests — verify turn parsing, event broadcasting, WebChat progress display                                            | ◻ Pending |
| 38  | OB-1053 | OB-F31  | Implement `checkpointSession()` on `MasterManager` — serialize pending workers, accumulated results, message context to `sessions` table | ◻ Pending |
| 39  | OB-1054 | OB-F31  | Implement `resumeSession()` on `MasterManager` — restore state from `sessions` table and continue processing                             | ◻ Pending |
...
| #   | Task ID | Finding | Description                                                                                                                                                                                                                                                                                                               | Status    |
| --- | ------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------- |
| 41  | OB-1060 | —       | Update `ARCHITECTURE.md` — fix `.openbridge/` folder spec (add `context/memory.md`, `openbridge.db` documentation, correct "v0.1.0 will migrate to SQLite" → "v0.0.2 migrated to SQLite"), add DotFolderManager prompt library methods, conversation history commands                                                     | ◻ Pending |
| 42  | OB-1061 | —       | Update `ROADMAP.md` — mark Phases 51–55 with task counts and status, move implemented backlog items (OB-820, OB-821, OB-822, OB-836, OB-930, OB-931) from backlog to shipped, update version milestones table                                                                                                             | ◻ Pending |
| 43  | OB-1062 | —       | Update `CHANGELOG.md` — add [Unreleased] entries for all new features: prompt library (Phase 51), `memory.md` conversation continuity (Phase 52), `/history` command + REST endpoints (Phase 53), schema versioning (Phase 54), worker streaming + checkpointing (Phase 55). Add Fixed entries for OB-F27 through OB-F35. | ◻ Pending |
