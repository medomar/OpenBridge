...
> **Pending:** 176 | **In Progress:** 0 | **Done:** 0 (1063 archived)
...
| ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------- | ------- |
| OB-1618 | Add `observations` table to SQLite — `id`, `session_id`, `worker_id`, `type`, `title`, `narrative`, `facts` (JSON), `concepts` (JSON), `files_read` (JSON), `files_modified` (JSON), `created_at` | `src/memory/database.ts`                | Pending |
| OB-1619 | Add migration (version 9) for `observations` table with idempotent guard check                                                                                                                    | `src/memory/migration.ts`               | Pending |
...
| Task    | What                                                                                                                                                             | Key File                                    | Status  |
| ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------- | ------- |
| OB-1625 | Create `observation-extractor.ts` — parse worker text output into structured `Observation` objects using regex + heuristics (file paths, function names, errors) | `src/master/observation-extractor.ts` (new) | Pending |
...
| Task    | What                                                                                                                                       | Key File                                | Status  |
| ------- | ------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------- | ------- |
| OB-1631 | Define `WorkerSummary` Zod schema: `{ request, investigated, completed, learned, next_steps, files_modified, files_read, error_summary? }` | `src/types/agent.ts`                    | Pending |
...
| Task    | What                                                                                                                                                      | Key File                                | Status  |
| ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------- | ------- |
| OB-1666 | Create `SessionCompactor` class — monitors Master session turn count via `agent_activity` tracking                                                        | `src/master/session-compactor.ts` (new) | Pending |
...
| OB-1679 | Increment `retrieval_count` on each chunk access in `hybridSearch()` and `getDetails()`                                                                   | `src/memory/retrieval.ts`   | Pending |
| OB-1680 | Add `/stats` chat command — show exploration ROI: "Explored with ~50K tokens, saved ~200K tokens across 15 retrievals (4x ROI)"                           | `src/core/router.ts`        | Pending |
...
| Task    | What                                                                                                   | Key File                                       | Status  |
| ------- | ------------------------------------------------------------------------------------------------------ | ---------------------------------------------- | ------- |
| OB-1682 | Tests: compaction trigger threshold, identifier extraction, summary generation, retry on failure       | `tests/master/session-compactor.test.ts` (new) | Pending |
...
| OB-1720 | Add `/whoami` chat command — shows user their role, channel, allowed actions, daily cost usage, and consent mode                                                     | `src/core/router.ts`         | Pending |
...
| Task    | What                                                                                                                                | Key File                                              | Status  |
| ------- | ----------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------- | ------- |
| OB-1726 | Define `DocumentSkill` type extending skill system — `name`, `description`, `fileFormat`, `toolProfile`, `npmDependency`, `prompts` | `src/types/agent.ts`                                  | Pending |
